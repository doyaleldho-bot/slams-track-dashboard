...
const LoginPage = () => {
    const [form, setForm] = useState({ userId: "", password: "", institution: "" });
    const [showPassword, setShowPassword] = useState(false);
    const [errors, setErrors] = useState({ userId: "", password: "", institution: "" });
  ...
  fetchCategories();
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        if (!form.institution.trim()) newErrors.institution = "Select an institution.";

        setErrors(newErrors);
        if (newErrors.userId || newErrors.password || newErrors.institution) { return; }
                ...
                <div className="grid gap-10 lg:grid-cols-[1.1fr_0.9fr] items-center">
                    <div className="mx-auto w-full max-w-md rounded-[28px] bg-white/95 p-8 shadow-[0_24px_80px_rgba(15,23,42,0.12)] ring-1 ring-slate-200">
                        <div className="mb-8 text-center">
                            ...
                            <h1 className="mt-6 text-2xl font-semibold text-slate-900">Login</h1>
                        </div>
                        ...
                        <form className="space-y-6" onSubmit={handleSubmit} noValidate>
                            <div>
                                ...
                                />
                                {errors.userId && <p className="mt-2 text-xs text-red-500">{errors.userId}</p>}
                            </div>
                            ...
                            <div>
                                ...
                                {errors.institution && <p className="mt-2 text-xs text-red-500">{errors.institution}</p>}
                            </div>
                    ...
                    <div className="relative overflow-hidden rounded-[32px] bg-white/95 p-10 shadow-[0_24px_80px_rgba(15,23,42,0.08)] ring-1 ring-slate-200">
                        ...
                        <div className="relative z-10 space-y-6">
                            <p className="text-sm uppercase tracking-[0.3em] text-slate-500">SLAMS Track</p>
                            <h2 className="text-3xl font-semibold text-slate-900 sm:text-4xl">Integrated academic and administrative management.</h2>
                            <p className="max-w-xl text-sm leading-7 text-slate-600">
                                SLAMS Track is an integrated academic and administrative management platform designed to simplify and streamline daily educational operations.
                            </p>
                            <p className="max-w-xl text-sm leading-7 text-slate-600">
                                It helps institutions efficiently manage student admissions, attendance, timetable scheduling, teacher management, payroll, fee collections, reports, and overall academic activities from a centralized system.
                            </p>
                            ...
                            <p className="text-center text-xs uppercase tracking-[0.3em] text-slate-400">Version 1.0 • Published on 21-05-2026</p>
                        </div>
                    </div>
                </div>
            </div>
